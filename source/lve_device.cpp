...
namespace lve {
	...
	{
		std::string sev;
		if      (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT) sev = "VERBOSE";
			...
			LOG_VULKAN("[DebugCallback:ERROR  ]\n\n" << pCallbackData->pMessage << "\n");
			return VK_FALSE;
		}
		else sev = "UNKNOWN";

		LOG_VULKAN("[DebugCallback:" << sev << "] " << pCallbackData->pMessage);
		return VK_FALSE;
	}
		...
		VkDebugUtilsMessengerEXT* pDebugMessenger)
	{
		...
		if (func != nullptr) {
			LOG_VULKAN("DebugMessanger created.");
		...
		const VkAllocationCallbacks* pAllocator)
	{
		...
		if (func != nullptr) {
			LOG_VULKAN("DebugMessanger destroyed.");
	...
	LveDevice::~LveDevice()
	{
		...
		LOG_VULKAN("Command pool destroyed.");
		vkDestroyDevice(m_device, nullptr);
		LOG_VULKAN("Logical device destroyed.");
		...
		LOG_VULKAN("Surface destroyed.");
		vkDestroyInstance(m_instance, nullptr);
		LOG_VULKAN("Instance destroyed.");
	}
	...
	void LveDevice::PickPhysicalDevice()
	{
		...
		LOG_VULKAN("Found " << deviceCount << " physical devices.");
		std::vector<VkPhysicalDevice> devices(deviceCount);
		vkEnumeratePhysicalDevices(m_instance, &deviceCount, devices.data());

		for (const auto& device : devices) {
			vkGetPhysicalDeviceProperties(device, &m_properties);
			LOG_VULKAN("Checking device: '" << m_properties.deviceName << "'");
...
//		vkGetPhysicalDeviceProperties(m_physicalDevice, &m_properties);
		LOG_VULKAN("Selected physical device: '" << m_properties.deviceName << "'");
	}
	...
	void LveDevice::CreateLogicalDevice()
	{
		...
		LOG_VULKAN("Logical device created.");
	...
	void LveDevice::CreateCommandPool()
	{
		...
		LOG_VULKAN("Command pool created.");
	}
...
//			VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT |
	...
	void LveDevice::HasGflwRequiredInstanceExtensions()
	{
		...
		LOG_VULKAN("Available instance extensions: " << extensionCount);
		std::unordered_set<std::string> available;
		for (const auto& extension : extensions) {
			available.insert(extension.extensionName);
		}

		LOG_VULKAN("Required instance extensions:");
