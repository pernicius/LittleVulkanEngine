	...
	VkPresentModeKHR LveSwapChain::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes)
	{
		for (const auto& availablePresentMode : availablePresentModes) {
			if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
				LOG_VULKAN("Present mode: Mailbox");
		...
		//     LOG_VULKAN("Present mode: Immediate");
		//     return availablePresentMode;
		//   }
		// }

		if (m_device.IsExtensionEnabled(VK_KHR_PRESENT_MODE_FIFO_LATEST_READY_EXTENSION_NAME)) {
			for (const auto& availablePresentMode : availablePresentModes) {
				if (availablePresentMode == VK_PRESENT_MODE_FIFO_LATEST_READY_KHR) {
					LOG_VULKAN("Present mode: V-Sync (Fifo-Latest)");
					return availablePresentMode;
				}
			}
		}

		LOG_VULKAN("Present mode: V-Sync (Fifo)");
